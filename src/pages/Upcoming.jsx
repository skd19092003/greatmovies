...
import { getAnnouncedUpcoming } from '../services/tmdb'
...
export default function Upcoming() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const data = await getAnnouncedUpcoming({ page }, { signal: controller.signal, timeout: 10000 })
        if (ignore) return
        ...
        // Use API-provided announced list as-is (no client filtering)
        let results = apiResults
        if (results.length === 20 && page < cappedTotalPages) {
          try {
            const nextData = await getAnnouncedUpcoming({ page: page + 1 }, { signal: controller.signal, timeout: 8000 })
          ...
          } catch { /* ignore to keep UX smooth */ }
        }
  ...
  return (
    <div id="upcoming-page" className="page-content">
      <div className="container-fluid px-2 px-sm-3">
        <div className="text-center mb-4">
          <h1 className="display-5 fw-bold text-gradient mb-2">Upcoming Movies</h1>
          <p className="text-muted">Movies that are announced and releasing soon</p>
        </div>
      </div>
      ...
      <div id="movie-box" className="movie-grid">
        ...
        {movies.length === 0 && !loading && (
          <div className="text-center text-muted py-5">No upcoming movies found</div>
        )}
        {movies.map((m) => (
          <MovieCard key={m.id} movie={m} showReleaseDate />
        ))}
      </div>
